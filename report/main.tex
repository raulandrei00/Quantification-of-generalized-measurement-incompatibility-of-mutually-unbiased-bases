...
\newcommand{\supervisor}[0]{Marc-Olivier Renou, Lucas-Amadeus Tendick}
...
Throughout generating these numerical results, we will provide a more general framework for quantifying these robustness thresholds by 
...
Quantum theory departs from classical physics already at the level of
measurement: not every collection of observables can be read out at the same
...
(see Definitions~\ref{def:jm}--\ref{def:incomp}). This definition reduces to
...
\begin{equation}
  ...
  \qquad j \in [k],\; a \in [d],
\end{equation}
and one asks for the largest noise parameter $\eta^\star$ below which the family
becomes jointly measurable. This \emph{white-noise robustness} is the figure of
...
compatibility scenarios such as the $(k-1)$-wise compatibility of $k$ MUBs.
...
with respect to two observables at once. It was later realised that Hermitian
operators are too restrictive to capture all physical measurements, and the
...
\{2,3,5,7\}$, where the Wootters--Fields construction provides explicit complete
...
The goal of this project is to study the numerical properties of the noise-robustness thresholds for the joint measurability and for the general 
$k$-wise incompatibility of MUB measurements. Towards this, we will reproduce the known results for joint measurability, while providing new numerical results for the generalised incompatibility scenarios. 
We will also derive an analytical lower bound for the $k$-wise incompatibility case and show tightness of this bound up to computational precision.
...
presentation of \cite{EP_FMA2S006}; we include them here for
...
\begin{definition}[Joint measurability] \label{def:jm}
  A set of quantum measurements (POVMs) $\{ M_{a|x} \}$ is said to be jointly measurable if there exists a single measurement (POVM) $\{G_\lambda\}$ such that the outcomes of $G$ can be post-processed to reproduce the statistics of each $M_i$.
...
\begin{definition}[Measurement incompatibility]\label{def:incomp}
  A set of quantum measurements (POVMs) $\{ M_{a|x} \}$ is said to be incompatible if it is not jointly measurable, i.e., if there does not exist a single measurement (POVM) $\{G_\lambda\}$ and a set of conditional probabilities $p(a|x, \lambda)$ such that for all $x$ and $a$, we have
  \begin{equation}
    M_{a|x} = \sum_{\lambda} p(a|x, \lambda) G_{\lambda}.
  \end{equation}
\end{definition}
...
\begin{aligned}
  \text{find}   \;\;& G_{\lambda_X \lambda_Z} \;\in\; \mathbb{C}^{2 \times 2}
                      ...
                      \in \{+, -\}^{2}, \\[2pt]
  \text{s.t.}   \;\;& G_{\lambda_X \lambda_Z} \;\succeq\; 0
...
\begin{wrapfigure}{r}{0.40\textwidth}
  ...
  \caption{Convex-combination view of joint measurability for three
           ...
           $JM_{13}$, $JM_{23}$ represent the sets of fully jointly
           measurable families for the respective pairs of measurements.}
...
setting of interest, which is done by summing over all $k$-tuples $\lambda_1 \cdots \lambda_k$ such that $\lambda_j = a$ for the setting $j$ and outcome $a$ of interest. Geometrically (Figure~\ref{fig:convex-3-measurements}), the feasibility of this SDP amounts to asking whether the noisy family $\{M^{\eta}_{a|j}\}$ lies in the convex hull of the single-branch jointly-measurable families.
.~\ref{fig:convex-3-measurements} illustrates geometrically why the noise-robustness threshold for full joint measurability is lower than the noise-robustness threshold for pairwise joint measurability: the set of fully jointly measurable families of MUBs (JM) is a strict subset of the convex hull of the pairwise jointly measurable families $(JM_{12}, JM_{13}, JM_{23})$.
...
\begin{aligned}
  ...
  \text{s.t.}   \;\;& G_{\lambda_1 \cdots \lambda_k} \;\in\; \mathbb{C}^{d \times d}
                      ...
                      \in [m]^{k}, \\[2pt]
                  \;\;& G_{\lambda_1 \cdots \lambda_k} \;\succeq\; 0
                  ...
                  \;\;& \sum_{\substack{\lambda_1, \dots, \lambda_k \\ \lambda_j = a}}
                        ...
                        \qquad\; \forall\, j \in [k],\ \forall\, a \in [m].
...
%\newpage
\subsection{Generalization of the robustness SDP to pairwise Jointly measurable hypergraphs}

Now that we have established certain robustness bounds for the joint measurability of $k$ MUBs, 
we can explore a more general notion of compatibility, which is the $(k-1)$-wise compatibility of $k$ MUBs.
More specifically, we are interested in finding a strategy of $(k-1)$ measurements that would yield a valid and identically distributed measurement for each of the $k$ MUBs, and we want to find the noise-robustness threshold for this compatibility scenario.
This is a natural relaxation of the full joint measurability scenario, and it is quite clear, and proved in the following, that this specific program will yield higher robustness thresholds.
\newline 
...
when it cannot be written as a convex combination of $k-1$ 
measurements that are pairwise compatible on different
partitions. By this, we mean choosing a measurement strategy in which we probabilistically choose a pair of measurements
$i \neq j$ with $i, j \leq k$ and perform a joint measurement for that pair while measuring the rest individually.
\newline
More formally, we have a probability matrix $P_{ij}$ with $i, j \in \llbracket 0, k \rrbracket $ and $i \neq j$, such that $\sum_{i < j} P_{ij} = 1$, and we have a set of parent POVMs $\{G^{(ij)}_{\lambda}\}$ for each pair of measurements $i \neq j$, such that the marginals of $G^{(ij)}_{\lambda}$ recover the noisy MUB measurements at settings $i$ and $j$.
\newline
The baseline program becomes: 
\newline
\begin{center}
\begin{align*}
&\text{find}      & & J_{a|x}^{(ij)},\ p_{ij},\ E_\lambda^{(ij)}\ \text{for all } i,j \in \llbracket 0,k \rrbracket,\ i<j \\
...
\newline
Notice that we cannot yet use convex optimization to solve this program, since the constraints are not \textbf{linear} due to the product of \textbf{variables} $p_{ij}$ and $J_{a|x}^{(ij)}$ in the decomposition of $M_{a|x}$.
...
add the maximization constraint and 
\begin{center}
  \begin{equation}
    \begin{aligned}
...
&\text{such that} & & E_\lambda^{(ij)} \geq 0, \sum_\lambda E_\lambda^{(ij)} = \frac{\mathds{1}}{d} \sum_\lambda Tr(E_\lambda^{(ij)}),\ \forall i,j \in \llbracket 0,k \rrbracket,\ i<j, \\
...
\begin{equation}
\begin{aligned}
\eta^* &= \frac{1}{\binom{k}{2}} \left( \binom{k-1}{1} \eta^{\star}(2, d) + \left( \binom{k}{2} - \binom{k-1}{1} \right) \cdot 1 \right) 
\end{aligned} \end{equation}
By simplifying, we get:
\begin{equation}
\begin{aligned}
\eta^* &= \frac{2}{k(k-1)} ((k-1) \eta^*(2, d) + \frac{k(k-1)}{2} - (k - 1)) \\ 
&= \frac{2}{k} (2 \eta^*(2, d) + k - 1) \\
&= 1 + \frac{1}{k} (\frac{2 + \sqrt{d}}{\sqrt{d} + 1} - 2) \\
&= 1 - \frac{\sqrt{d}}{k(\sqrt{d} + 1)}
\end{aligned}
\end{equation} 

In the results section, we will see that, for the cases we compute numerical values, this lower bound is tight, up to computational precision, which leads us to 
...
Note that we are able to expand the second table much more than the first one, which at first could seem counter intuitive since the pairwise hypergraph is generating more constraints on the optimization problem.
...
is therefore $\binom{k}{2}\bigl(d^2 + (k-2)\,d\bigr) = \mathcal{O}(k^3 d^2)$,
which grows only \emph{polynomially} with $k$. The same gap shows up in
the marginal constraints, where the full-JM SDP sums $d^{k-1}$ matrices
per equality while the pairwise SDP sums only $d$ matrices per branch.
Concretely, for $(k=8,\, d=7)$ the full-JM SDP would require
$7^{8} \approx 5.76 \cdot 10^{6}$ matrix variables (intractable on any
desktop), while the pairwise SDP requires
$\binom{8}{2}(7^{2} + 6\cdot 7) = 2{,}548$ matrix variables and solves
in seconds; this is why the second table can be populated for every
$(k, d)$ in our range whereas the first cannot.
...
\begin{table}[h]
  ...
  \caption{Numerical values for measurement compatibility:
           ...
           $\eta^{\star}(d+1, d) = 1/\sqrt{d+1}$); cells marked ``?''
           are to be obtained by direct SDP optimisation. ``---'' marks
...
\begin{table}[h]
  \centering
  \caption{Numerical values for $(k\!-\!1)$ compatibility:
           noise-robustness thresholds for the compatibility hypergraph
           consisting of all $(k\!-\!1)$-element subsets of $k$ MUBs in
           prime dimension $d$ (the natural relaxation of full joint
           measurability discussed in
           Appendix~\ref{app:hypergraph-helpers}). ``---'' marks cells
           with $k > d+1$.}
  \label{tab:k-minus-1-compat}
  \renewcommand{\arraystretch}{1.4}
  \begin{tabular}{c|ccccc}
    ...
    $3$ & 0.8047 & 0.7887 & 0.7697 & 0.7581 & \\
    $4$ &    ---     &  0.8415  &  0.8273  &  0.8186  & \\
    $5$ &    ---     &    ---   &  0.8618  &  0.8549  & \\
    $6$ & --- & --- &0.8848  &  0.8790 & \\
    $7$ & --- & --- & --- & 0.8963 & \\
    $8$ & --- & --- & --- & 0.9093 & \\
...
In this paper we have reviewed the recent literature on the joint measurability of quantum measurements, while reproducing some of the known numerical results on the noise-robustness thresholds for the joint measurability of $k$ MUBs in prime dimension $d$ by implementing the standard robustness SDP. 
We then generalised this program to arbitrary compatibility hypergraphs, and used this generalisation to obtain new numerical results on the noise-robustness thresholds for the $(k)$-wise incompatibility case. We found an analytical lower bound for this program, which is tight for the cases where we computed numerical values.
\newline
\paragraph{Going further,} a natural next step would be to prove tightness of this lower bound by implementing the Dual SDP, which yields an (analytical) upper bound.
...
Section~\ref{sec:sdp-2qubit}. It is built on top of CVXPY and delegates
